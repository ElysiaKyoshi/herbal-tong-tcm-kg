...
async def zhongyi_intent_node(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    await put_msg_sentence_content(user_id, "完成识别是否是中医的意图识别")
    return state

if __name__ == '__main__':
    import asyncio
    state = AgentState(input="请问人参有什么功效")
    result = asyncio.run(zhongyi_intent_node(state=state, config={"configurable": {"thread_id": "test_user"}}))
    print(result)
