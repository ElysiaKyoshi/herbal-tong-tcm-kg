...
async def llm_direct_out_node(state: AgentState, config: RunnableConfig):
    ...
    await put_reply_content(user_id, model_answer)
...
if __name__ == '__main__':
    import asyncio
    result = asyncio.run(llm_direct_out_node(state=AgentState(input="历史菲尔兹奖获得者都有谁?"), config={"configurable": {"thread_id": "test_user"}}))
    print(result)